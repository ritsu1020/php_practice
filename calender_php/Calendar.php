...
class Calendar {
  ...
  private function _getTail() {
    $tail = '';
    $lastDayPrevMonth = new \DateTime('last day of' . $this->yearMonth . '-1 month');
    while ($lastDayPrevMonth < 6) {
      $tail = sprintf('<td class="gray">%d</td>', $lastDayPrevMonth->format('d')) .$tail;
  ...
  private function _getBody() {
    ...
    $period = new \DatePeriod(
      ...
      new \DateTime('first dat of' . $this->yearMonth . '+1 month');
    );
  ...
  private function _getHead() {
    $head = '';
    $firstDayOfNextMonth = new \DateTime('first dat of' . $this->yearMonth .'+1 month');
